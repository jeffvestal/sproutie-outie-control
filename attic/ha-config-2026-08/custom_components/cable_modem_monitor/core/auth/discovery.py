    ...
    def auth_patterns(self) -> dict:
        ...
        if self._auth_patterns is None:
            try:
                ...
                self._auth_patterns = {
                    "form": {
                        "username_fields": <redacted>
                        "password_fields": <redacted>
                        "actions": [],
                        "encodings": [],
                    },
...
<redacted>
        ...
        form_data: dict[str, str] = {
            form_config.username_field: <redacted>
            form_config.password_field: <redacted>
...
<redacted>
    ...
    def _get_url_token_config(self, parser: <redacted>
...
<redacted>
            ...
            _LOGGER.debug("Failed to load modem.yaml URL token config: <redacted>
...
<redacted>
        arguments=base64(urlencode("username=X:<redacted>
        """
        import base64
        from urllib.parse import quote
...
<redacted>
        ...
        form_data: dict[str, str] = {
            form_config.credential_field: <redacted>
            **form_config.hidden_fields,
        }
...
<redacted>
    ...
    def _is_combined_credential_form(self, html: <redacted>
    ...
    def _parse_combined_form(self, html: str) -> DiscoveredFormConfig | None:
        ...
        return DiscoveredFormConfig(
            ...
            credential_format="username={username}:<redacted>
    ...
    def _detect_auth_hints_from_html(self, html: str) -> dict[str, str] | None:
        ...
        # - URL token auth pattern: <redacted>
...
<redacted>
        ...
        for name, _inp in text_inputs:
            if name in known_fields:
                _LOGGER.debug("Found username field by exact match: <redacted>
                return str(name)
...
<redacted>
                _LOGGER.debug("Found username field by heuristic: <redacted>
                return str(name)

        # 3. Fallback: first text input
        if text_inputs:
            name = text_inputs[0][0]
            _LOGGER.debug("Using first text input as username field: <redacted>
            return str(name)
...
<redacted>
    ...
    def _detect_password_encoding(self, html: <redacted>
        ...
        for encoding_entry in self.encoding_patterns:
            ...
            if pattern and re.search(pattern, html, re.IGNORECASE | re.DOTALL):
                _LOGGER.debug(
                    "Detected %s password encoding (index pattern: <redacted>
                    encoding_type,
                    pattern[:40],
                )
        ...
        base64_patterns_fallback = [
            ...
            r"loginPassword\.value\s*=\s*(?:<redacted>
        ...
        for pattern in base64_patterns_fallback:
            if re.search(pattern, html, re.IGNORECASE | re.DOTALL):
                _LOGGER.debug("Detected base64 password encoding (fallback pattern: <redacted>
                return "base64"

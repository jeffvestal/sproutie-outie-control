...
#
# Source: modems/arris/sb8200/parser.py
...
v3.12.0+: Uses Fetcher architecture - parser only parses, never fetches.
All HTTP calls are handled by HTMLFetcher with URL token auth support.
"""
...
class ArrisSB8200Parser(ModemParser):
    ...
    - Credentials are base64-encoded as "username:<redacted>
    ...
    - Default credentials: <redacted>

    v3.12.0+: URL token handling moved to HTMLFetcher. Parser only parses.
    """
    ...
    def parse_resources(self, resources: dict[str, Any]) -> dict:
        ...
        Args:
            resources: Dict with fetched pages:
                - "/cmconnectionstatus.html" or "/" -> BeautifulSoup (main page)
                - "/cmswinfo.html" -> BeautifulSoup (product info page)

        Returns:
            Dict with downstream, upstream, system_info
        """
        # Get main page (connection status)
        main_soup = resources.get("/cmconnectionstatus.html") or resources.get("/")
        if not main_soup:
            _LOGGER.warning("SB8200: No main page found in resources")
            return {"downstream": [], "upstream": [], "system_info": {}}

        # Parse channel data from main page
        downstream_channels = self._parse_downstream(main_soup)
        upstream_channels = self._parse_upstream(main_soup)
        system_info = self._parse_system_info(main_soup)

        # Get product info page for uptime and version info
        info_soup = resources.get("/cmswinfo.html")
        if info_soup:
            product_info = self._parse_product_info(info_soup)
            system_info.update(product_info)
            _LOGGER.debug("SB8200: Parsed product info from cmswinfo.html")
        else:
            _LOGGER.debug("SB8200: cmswinfo.html not in resources, skipping product info")

        # Extract actual model from HTML
        model_name = self._extract_model(main_soup)
        if model_name:
    ...
    def parse(self, soup: BeautifulSoup, session=None, base_url=None) -> dict:
        ...
        Kept for backward compatibility. New code should use parse_resources().
        """
        # Build resources dict from legacy params
        resources: dict[str, Any] = {"/": soup}

        # If session provided, fetch additional pages the old way
        if session and base_url:
            try:
                # Note: This path is deprecated - fetcher should handle this
                _LOGGER.debug("SB8200: Legacy parse() path - session-based fetch")
                response = session.get(f"{base_url}/cmswinfo.html", timeout=10, verify=False)
                if response.status_code == 200:
                    resources["/cmswinfo.html"] = BeautifulSoup(response.text, "html.parser")
            except Exception as e:
                _LOGGER.debug("SB8200: Failed to fetch cmswinfo.html: %s", e)
